 ...
 *
 * Le modèle ne rend que **le corps** : la feuille de style vit ici. La lui
 * faire recopier coûtait trois mille jetons de sortie à chaque génération —
 * une minute de calcul pour du style identique d'une fois sur l'autre — et
 * laissait la mise en page dériver au gré des humeurs.
 *
 ...
 *
 * Un corps marqué est habillé de la feuille de style ; un document ancien,
 * qui porte déjà la sienne, est rendu tel quel.
 */
export function extraireRapportHtml(contenu: string): string {
  ...
  return `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8">`
    ...
    + `<style>${FEUILLE_STYLE}</style></head><body>${corps}</body></html>`
}
